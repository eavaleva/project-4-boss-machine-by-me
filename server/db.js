...
// Takes only the single argument for model name. Returns the array of elements in the database or null if an invalid argument is supplied to the function.
 ...
 * addToDatabase handles assigning .id properties to the instances. It does not check to make sure that valid inputs
 * are supplied, so you will have to add those checks to your routes if necessary. addToDatabase will return
 * the newly-created instance from the database.
 ...
 * The instance provided must have a valid .id property which will be used to match. updateInstanceInDatabase will
 * return the updated instance in the database or null with invalid inputs.
 ...
 * You will only need to use this function for a /api/meetings route.
 * */
